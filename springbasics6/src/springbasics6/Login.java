	...
	public ResponseEntity<MensajeRespuesta> login(@RequestParam(value = "idioma", required = false) String lang, @Valid @RequestBody LoginUsuario login_usuario, BindingResult result) {
		...
		MensajeRespuesta mensaje_respuesta = null;
				...
				if (hay_errores)
				{
					...
					String str_aux = mensajes.getMessage(result.getFieldError(), new Locale (lang)); //sorpresivamente, el método getFiledrror, devuelve el primer error, que no siempre es el mismo. Debe ser que el orden de validación es variable
				...
				} else
					{
						...
						try {
							...
							} catch (Throwable t) { //Estrategia mixta, capturo y propago para : 1) que quede constancia del error LOG y 2) para que el usuario sea informado propiamente
